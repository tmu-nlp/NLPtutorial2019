...
def train(filename: str):
    ...
    with open('hmm_percep', 'wb') as hmm_f,\
        open('possible_tags', 'wb') as p_f, \
        open('trainsition', 'wb') as t_f:
        hmm_f.write(dill.dumps(w))
        p_f.write(dill.dumps(possible_tags))
        t_f.write(dill.dumps(trainsition))
