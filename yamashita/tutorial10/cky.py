...
def print_Stree(sym_ij, best_edge, words):
    sym, i, _ = re.sub("[()]", "", sym_ij).split()
    i = int(i)
    if sym_ij in best_edge.keys():
        return f'({sym} {print_Stree(best_edge[sym_ij][0],best_edge, words)} {print_Stree(best_edge[sym_ij][1], best_edge, words)})'
    else:
...
def load_grammer(grammar_file):
    ...
    with open(grammar_file, 'r') as g_file:
        for line in g_file:
            lhs, rhs, prob = line.rstrip().split('\t')
...
def cky(grammar_file, input_file):
    nonterm, preterm = load_grammer(grammar_file)
    with open(input_file, 'r') as i_file:
        for line in i_file:
            words = line.rstrip().split()
            best_score = defaultdict(lambda: -10**9)
            best_edge = {}
                ...
                for i in range(j-1)[::-1]:
                    for k in range(i+1, j):
                        for sym, lsym, rsym, logprob in nonterm:
                            if best_score[f'{lsym} ({i} {k})'] > -10**9 and best_score[f'{rsym} ({k} {j})'] > -10**9:
            ...
            print(print_Stree(f'S (0 {len(words)})', best_edge, words))
