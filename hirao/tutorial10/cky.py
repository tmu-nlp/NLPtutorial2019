...
class CKY:
    def __init__(self, grammar_file: str):
        self.nonterm = []   # 非終端記号
        self.preterm = defaultdict(lambda: [])  # 事前確率P(L|R)
        # 文法の読み込み
        self.input_grammar(grammar_file)

    def input_grammar(self, file_name: str):
        """
        input file format
        lhs \t rhs \t prob \n
        ...
        """
        for rule in open(file_name):
    ...
    def run(self, line: str):
        ...
        # 存在しないもののlogprobは-infで初期化
        self.best_score = defaultdict(lambda: -10**9)
        self.best_edge = {}

        # 前終端記号を追加
        for i in range(len(self.words)):
            # 前終端記号が事前確率で与えられているもの
            # P(NP_PRP | i) = 0.4
            # iが与えられた時、NP_PRPである確率
            # そのlogをとる
            if self.preterm[self.words[i]] != []:
                for lhs, logprob in self.preterm[self.words[i]]:
                    self.best_score[f"{lhs} ({i} {i+1})"] = logprob

        # 非終端記号の組み合わせ
        for j in range(2, len(self.words)+1):
            for i in range(j-1)[::-1]:
                for k in range(i+1, j):
                    # iとjの間をkで区切った時のスコアを計算
                    for sym, lsym, rsym, logprob in self.nonterm:
                        # (lsym + rsym) -> sym　になる確率
                        # ex. (NP VP) -> S
                        # 超グラフを網羅的に探索、確率が最も高いエッジを保存する
                        if self.best_score[f"{lsym} ({i} {k})"] > -10**9 and self.best_score[f"{rsym} ({k} {j})"] > -10**9:
    ...
    def print_tree(self, sym_ij: str) -> str:
        # Sの子を再帰的に表示していく
...
""" test
input: test/08-inut.txt
grammar: test/08-grammar.txt
test: test/08-output.txt
...
""" wiki-en-short
input: data/tiki-en-short.tok
grammar: data/wiki-en-test.grammar
...
(S (S (NP (CD Two) (NP' (JJ shallow) (NNS approaches))) (VP (VBN used) (S_VP (TO to) (VP (VB train) (VP' (CC and) (VP' (ADVP_RB then) (VP_VB disambiguate))))))) (S' (VP (VBP are) (NP (NP (NNP Naïve) (NNP Bayes)) (NP' (NNS classifiers) (NP' (CC and) (NP (NN decision) (NNS trees)))))) (. .)))
